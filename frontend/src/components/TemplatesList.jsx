...
import CreateTemplateModal from "./CreateTemplateModal.jsx"; // ADDED

const TemplatesList = () => {
    const [templates, setTemplates] = useState([]);
    const [loading, setLoading] = useState(false);
    const [openCreate, setOpenCreate] = useState(false); // ADDED
    ...
    return (
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
            {/* Header row — added zIndex so nothing overlays this */}
            <div
                style={{
                    display: 'flex',
                    ...
                    zIndex: 5,               // ADDED
                    background: '#fff'       // ADDED (prevents blending under translucent headers)
                }}
            >
                <h3 style={{ margin: '0', color: '#333' }}>
                    📋 Estimate Templates ({templates.length})
                </h3>
                <button
                    onClick={() => setOpenCreate(true)} // ADDED
                    style={{
            ...
            {/* Spreadsheet modal (opens on button click) */}
            <CreateTemplateModal
                open={openCreate}
